...
async function logCritical(msg) { process.stdout.write(String(msg) + '\n'); await flushLogs(); }
async function logInfo(msg) { console.log(msg); await flushLogs(); }
async function logWarn(msg) { console.warn(msg); await flushLogs(); }
async function logError(msg) { console.error(msg); await flushLogs(); }
...
global.db.loadDatabase = async function () {
  const loadPromises = Object.keys(collections).map(async (category) => {
    const docs = await new Promise((resolve, reject) => {
      collections[category].find({}, (err, docs) => { if (err) return reject(err); resolve(docs); });
    });
    ...
    for (const doc of docs) {
      ...
      if (seenIds.has(originalId)) {
        await new Promise((resolve, reject) => {
          collections[category].remove({ _id: doc._id }, {}, (err) => { if (err) return reject(err); resolve(); });
        });
      } else {
...
const question = (texto) => {
  ...
  return new Promise((resolver) => {
    rl.question(texto, (respuesta) => { rl.clearLine(rl.input, 0); resolver(respuesta.trim()) })
  })
}
...
const backupCreds = async () => {
  if (!fs.existsSync(credsFile)) { console.log(await tr('[⚠] No se encontró el archivo creds.json para respaldar.')); return; }
...
const restoreCreds = async () => {
  ...
  if (backups.length === 0) { console.log('[⚠] No hay respaldos disponibles para restaurar.'); return; }
...
setInterval(async () => { await backupCreds(); console.log('[♻️] Respaldo periódico realizado.') }, 5 * 60 * 1000)
...
function clearTmp() {
  ...
  filenames.forEach(file => { const filePath = join(tmpDir, file); try { unlinkSync(filePath) } catch {} })
}
  ...
  try {
    ...
    for (const file of preKeys) {
      ...
      if (fileStats.mtimeMs < oneHourAgo) { 
        try { await unlink(filePath); console.log(chalk.green(`[🗑️] Pre-key antigua eliminada: ${file}`)); } catch {}
      } else {
...
async function purgeSessionSB() {
      ...
      if (statsDir.isDirectory()) {
        ...
        for (const file of preKeys) {
          ...
          if (fileStats.mtimeMs < oneDayAgo) { 
            try { await unlink(filePath); console.log(chalk.bold.green(`${lenguajeGB.smspurgeOldFiles1()} ${file} ${lenguajeGB.smspurgeOldFiles2()}`)); } catch {}
          }
        }
      }
    }
  ...
  } catch (err) { console.log(chalk.bold.red(lenguajeGB.smspurgeSessionSB3() + err)); }
}
...
async function purgeOldFiles() {
  ...
  for (const dir of directories) {
    try {
      if (!fs.existsSync(dir)) { console.log(chalk.yellow(`[⚠] Carpeta no existe: ${dir}`)); continue; }
