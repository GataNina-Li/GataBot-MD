...
let mdFormat = (depth = 4) => (match, emphasisType, emphasisText, backtickedText, codeText, quotedText, normalText) => {
    let formattedText = normalText || '';

    if (emphasisType) {
        formattedText = applyEmphasis(emphasisType, formattedText);
    } else if (backtickedText) {
        formattedText = chalk.bgGray.white(backtickedText);
    } else if (codeText) {
        formattedText = codeText;
    } else if (quotedText) {
        formattedText = chalk.cyan(quotedText);
    }

    return formattedText;
};

function applyEmphasis(emphasisType, text) {
    switch (emphasisType) {
        case '_':
