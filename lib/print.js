...
let mdFormat = () => (match, emphasis, quoted, text) => {
    let types = {
        '_': 'italic',
        '*': 'bold',
        '~': 'strikethrough'
    ...
    let formattedText = quoted || text
    if (quoted !== undefined) {
        formattedText = chalk.bgGray.white[types[emphasis]](quoted)
    } else if (emphasis && types[emphasis]) {
        formattedText = chalk[types[emphasis]](formattedText);
    }
