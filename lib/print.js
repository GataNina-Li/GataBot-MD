...
┃ ❖ Horario: ${chalk.black.bgGreen('%s')} | ${chalk.red('%s [%s %sB]')}
┃ ❖ ${chalk.black(chalk.cyan('%s'))}
...
//m.chat + (chat ? ' ~' + chat : ''),
(m.chat.includes("@g.us") ? `Grupo: ${m.chat} | ${conn.getName(m.chat)}` : `Privado: ${m.chat} | ${chalk.white(`${user.name ? user.name : conn.getName(m.chat) }`)} `),
