...
┃ ❖ ${chalk.white.bold(mid.idioma_code === 'es' ? 'Usuario citado:' : 'Cited user')} %s
╰━━━━━━━━━━━━━━𖡼`.trim(),
...
m.message?.extendedTextMessage?.contextInfo?.quotedMessage ? chalk.bold(`${conn.getName(m.sender) ?? user.name ?? 'Este usuario'}`) + ' respondió a ' + chalk.bold(`${conn.getName(m.message?.extendedTextMessage?.contextInfo?.participant) ?? m.message?.extendedTextMessage?.contextInfo?.participant ?? 'Usuario desconocido'}`) : 'No'
)
