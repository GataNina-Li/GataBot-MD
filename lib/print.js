...
let mdFormat = (depth = 4) => (_, type, text1, text2, monospace, quoted, normalText) => {
    let types = {
        '_': 'italic',
        '*': 'bold',
        '~': 'strikethrough'
    ...
    if (quoted !== undefined) {
        ...
        formattedText = chalk.cyan(text1);
    } 
    
    else if (monospace !== undefined) {
        formattedText = monospace;
    } 
    ...
    let formatted = !types[type] || depth < 1 ? formattedText : chalk[types[type]](formattedText.replace(mdRegex, mdFormat(depth - 1)))
    return formatted;
}
if (log.length < 1024)
