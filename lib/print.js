...
let mdFormat = (depth = 4) => {
    ...
    return (_, type, text, monospace) => {
        text = text || monospace;

        // Aplicar el efecto 'bgGray.dim' a las líneas que comienzan con '>'
        let dimmedText = text.replace(gtRegex, (match, g1) => {
            return chalk[types['dim']](g1); // Utilizar types['bgGray.dim']
        });

        // Aplicar los demás efectos de formato
        let formatted = !types[type] || depth < 1 ? dimmedText : chalk[types[type]](dimmedText.replace(/`/g, '').replace(mdRegex, mdFormat(depth - 1)));
