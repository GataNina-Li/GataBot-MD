...
m.message?.extendedTextMessage?.contextInfo?.quotedMessage ? (m.message?.extendedTextMessage?.contextInfo?.participant == m.sender ? `${conn.getName(m.sender) ?? user.name ?? 'Este usuario'} respondió a su propio mensaje.` :
...
: ' envío mensaje en el canal ' + chalk.bold(`${conn.getName(m.message?.extendedTextMessage?.contextInfo?.participant)}`)))
: ''
)
