...
m.message?.extendedTextMessage?.contextInfo?.quotedMessage ? '┃ ❖ ' + chalk.bold(`${conn.getName(m.sender) ?? user.name ?? 'Este usuario'}`) + ' respondió a ' + chalk.bold(`${conn.getName(m.message?.extendedTextMessage?.contextInfo?.participant) ?? m.message?.extendedTextMessage?.contextInfo?.participant ?? 'Usuario desconocido'}`) : ''
)
