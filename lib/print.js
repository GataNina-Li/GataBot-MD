...
return chalk.bold(`${name_info ? name_info : 'Alguien'} [${PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international')}]`)
}).join(', ')
...
formatMessageTypes(m.mtype) //m.mtype ? m.mtype : 'No especificado'  //? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : ''
)
...
function formatMessageTypes(messageStubType, mid) {
switch (messageStubType) {
case 'conversation':
return 'Conversación'
case 'imageMessage':
...
default:
return mid.idioma_code === 'es' ? messageStubType || 'No especificado' : formaTxt(messageStubType) || 'Not specified'
}}

function formaTxt(messageType) {
