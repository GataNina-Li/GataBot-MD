...
//let mdRegex = /(?<=(?:^|[\s\n])\S?)(?:([*_~`])(?!`)(.+?)\1|```((?:.|[\n\r])+?)```|`([^`]+?)`|(^>[^\n]*))(?=\S?(?:[\s\n]|$))/g
const mdRegex = /(?<=(?:^|[\n])\S?)(?:([*_~`])(?!`)(.+?)\1|```((?:.|[\n\r])+?)```|`([^`]+?)`)|(^>[^\n]*)/gm
let mdFormat = (depth = 4) => (_, type, text, monospace, gtLine) => {
    let types = {
        '_': 'italic',
        '*': 'bold',
        '~': 'strikethrough',
    ...
    let formattedText = text ?? monospace ?? gtLine;

    
    if (gtLine && gtLine.startsWith('>')) {
        formattedText = formattedText.substring(1); // Elimina el '>' inicial
        return chalk.dim(formattedText);
    }

    
    let formatted = !types[type] || depth < 1 ? formattedText : chalk[types[type]](formattedText.replace(/`/g, '').replace(mdRegex, mdFormat(depth - 1)));
