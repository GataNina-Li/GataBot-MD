...
m.message?.extendedTextMessage?.contextInfo?.quotedMessage ? '┃ ❖ ' + chalk.bold(`${conn.getName(m.sender) ?? user.name ?? 'Este usuario'}`) + m.message?.extendedTextMessage?.contextInfo?.participant.includes("newsletter") ? ' envío mensaje en el canal ' + chalk.bold(`${conn.getName(m.message?.extendedTextMessage?.contextInfo?.participant)}`) : ' respondió a ' + chalk.bold(`${conn.getName(m.message?.extendedTextMessage?.contextInfo?.participant) ?? m.message?.extendedTextMessage?.contextInfo?.participant ?? 'Usuario desconocido'}`) : ''
)
