...
//(text.includes('`') ? chalk.bgGray.white(text.replace(mdRegex, mdFormat(depth - 1))).replace(/`/g, '') : 
...
let formatted = !types[type] || depth < 1 ? text :
    (text.includes('`') && types[type] ?
        ...
        (text.includes('`') ?
            ...
            chalk[types[type]](text.replace(mdRegex, mdFormat(depth - 1).replace(/`([^`]*)`/g, (_, content) => chalk.bgGray.white(content))))))
return formatted
