...
let fraseAEliminar = "Closing session: SessionEntry"
let regexT = new RegExp('.*' + fraseAEliminar + '[^]*')
log = log.replace(regexT, "Mensaje omitido")
...
console.log(m.error != null ? chalk.red(log) : m.isCommand ? log : log) //chalk.yellow(log) : log)
}
