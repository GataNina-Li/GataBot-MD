...
console.log(`
❖ Bot: ${chalk.cyan.bold('%s')} 
❖ Horario: ${chalk.white.bgGreen('%s')} | ${chalk.red('%s [%s %sB]')}
❖ Acción: ${chalk.black(chalk.cyan('%s'))}
❖ Usuario: ${chalk.white('%s')} | ${chalk.yellow('%s%s')}
❖ ${chalk.magenta('%s')}
❖ Tipo de mensaje: ${chalk.black(chalk.red('%s'))}`.trim(),
...
formatMessageStubType(WAMessageStubType[m.messageStubType], name_user),
filesize,
...
//m.chat + (chat ? ' ~' + chat : ''),
(m.chat.includes("@g.us") ? `Grupo: ${m.chat} | ${conn.getName(m.chat)}` : `Privado: ${m.chat} | ${conn.getName(m.chat)}`),
