...
'~': 'strikethrough',
//'`': 'bgGray'
'`': text => chalk.bgGray(chalk.monospace(text))
}
text = text || monospace
//let formatted = !types[type] || depth < 1 ? text : chalk[types[type]](text.replace(/`/g, '').replace(mdRegex, mdFormat(depth - 1)))
let formatted = !types[type] || depth < 1 ? text : (typeof types[type] === 'function' ? types[type](text.replace(/`/g, '').replace(mdRegex, mdFormat(depth - 1))) : chalk[types[type]](text.replace(/`/g, '').replace(mdRegex, mdFormat(depth - 1))))
return formatted
}   
