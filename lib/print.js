...
let mdFormat = (depth = 4) => (_, type, text, quoted) => {
    let types = {
        '_': 'italic',
        '*': 'bold',
        '~': 'strikethrough'
    ...
    let formattedText = text || quoted;
    if (quoted && text) {
        formattedText = chalk.bgGray.white.bold.italic(text);
    } else if (quoted && type === '*') {
        formattedText = chalk.bgGray.white.bold(text);
    } else if (quoted) {
        formattedText = chalk.bgGray.white.italic(text);
    } else if (types[type]) {
