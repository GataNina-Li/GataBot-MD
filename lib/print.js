...
┃ ❖ ${chalk.white.bold('Horario:')} ${chalk.black.bold.bgGreen('%s')}
┃ ❖ ${chalk.white.bold('Acción:')} ${formatMessageStubType(m.messageStubType, name_user)}
┃ ❖ ${chalk.white.bold('Usuario:')} ${chalk.white('%s')} / ${chalk.yellow('%s%s')} / ${chalk.bgBlueBright.bold(user.role.replace(/\*/g, ''))} / Premium » ${user?.premiumTime > 0 ? '✅' : '❌'}
┃ ❖ ${chalk.magenta('%s')}
