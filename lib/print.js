...
let formatted = !types[type] || depth < 1 ? text : 
        (text.includes('`') && types[type] ? 
            chalk.bgGray.white(chalk[types[type]](text.replace(mdRegex, mdFormat(depth - 1)))) : 
            (text.includes('`') ? 
                chalk.bgGray.white(text.replace(mdRegex, mdFormat(depth - 1))) : 
...
let mdFormat = (depth = 4) => (_, type, text1, text2, quoted) => {
    let types = {
        '_': 'italic',
        '*': 'bold',
        '~': 'strikethrough'
    ...
    let text = text1 || text2 || quoted;
    
    if (quoted !== undefined) {
        if (quoted === '') {
            return chalk.bgGray.white('');
        }
        text = chalk.bgGray.white(text); // Aplicar bgGray.white al texto rodeado por una comilla invertida
    } else {
    
        let formatted = !types[type] || depth < 1 ? text : 
            (text.includes('`') && types[type] ? 
                chalk.bgGray.white(chalk[types[type]](text.replace(mdRegex, mdFormat(depth - 1)))).replace(/`/g, '') : 
                (text.includes('`') ? 
                    chalk.bgGray.white(text.replace(mdRegex, mdFormat(depth - 1).replace(/`/g, ''))) : 
                    chalk[types[type]](text.replace(mdRegex, mdFormat(depth - 1)))));
        return formatted
    }
};
