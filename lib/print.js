...
let mdFormat = (depth = 4) => (_, type, text, monospace, dimLine) => {
    let types = {
        '_': 'italic',
        '*': 'bold',
        '~': 'strikethrough',
    ...
    // Si la línea empieza con '>', aplicamos el efecto 'dim' a toda la línea
    if (dimLine) {
        return chalk.dim(dimLine)
    }

    // Aplicamos el formato según el tipo de markdown
    let formatted = !types[type] || depth < 1 ? text : chalk[types[type]](text.replace(/`/g, '').replace(mdRegex, mdFormat(depth - 1)));
    return formatted;
};
