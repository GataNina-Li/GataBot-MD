...
sendGroupV4Invite: {
  ...
  const sections = [
    {
      title: !nullish(lister[1]) && lister[1] || '',
      rows: []
    }
  ];

  let rowTitle = lister[2], rowId = lister[3], description = lister[4]
  ...
  for (let i = 1; i < lister.length; i++) {
    if (Array.isArray(lister[i])) {
      const sectionData = lister[i];
      const sectionTitle = sectionData.shift(); 

      const sectionRows = [];
      while (sectionData.length > 0) {
        const rowTitle = sectionData.shift();
        const rowDescription = sectionData.shift();
        const rowId = sectionData.shift();

        sectionRows.push({
          title: !nullish(rowTitle) ? rowTitle : '',
          description: !nullish(rowDescription) ? rowDescription : '',
