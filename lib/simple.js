...
conn.sendMessage(prefCmd, chatUpdate)
}}}})*/

conn.sendMessage = async(text, chatUpdate) => {
...
conn.ev.on('messages.upsert', async chatUpdate => {
    if (chatUpdate.update.pollUpdates && chatUpdate.key.fromMe) {
        const pollCreation = await getMessage(chatUpdate.key);
        if (pollCreation) {
            const pollUpdate = await getAggregateVotesInPollMessage({
                message: pollCreation,
                pollUpdates: chatUpdate.update.pollUpdates,
            });
            var toCmd = pollUpdate.filter(v => v.voters.length !== 0)[0]?.name;
            if (toCmd == undefined) return;
            var prefCmd = '.' + toCmd;
