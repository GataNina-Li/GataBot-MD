...
import { channelRD } from '../plugins/_content.js'
...
END:VCARD`.trim()
    ...
    return await conn.sendMessage(jid, {
      ...
      ...options
	    ...
	    return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text, contextInfo: { mentionedJid: null, forwardedNewsletterMessageInfo: { newsletterJid: channelRD.id, serverMessageId: '', newsletterName: channelRD.name }, forwardingScore: 9999999, isForwarded: true, ...options }}, { quoted, ...options })
            }
        },
