...
let canalesInfo_ = [
{ id: "120363160031023229@newsletter", name: "INFINITY-WA 💫" },
{ id: "120363169294281316@newsletter", name: "GB - UPDATE ✨" },
{ id: "120363203805910750@newsletter", name: "Tips sobre GataBot 🤩" } 
]
let indiceAleatorio_ = Math.floor(Math.random() * canalesInfo_.length)
let channelR_ = canalesInfo_[indiceAleatorio_]
return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text, contextInfo: { mentionedJid: null, forwardedNewsletterMessageInfo: { newsletterJid: channelRD_.id, serverMessageId: '', newsletterName: channelRD_.name }, forwardingScore: 1, isForwarded: true, ...options }}, { quoted, ...options })
}
},
