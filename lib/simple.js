...
sendGroupV4Invite: {
     ...
     */
    sendCarousel: {
      async value(jid, text = '', footer = '', text2 = '', messages, listSections, quoted, options) {
...
// Mapeamos listSections a la estructura deseada
const sections = listSections.map(([title, rows]) => ({
    title: title || '',
    rows: rows.map(([rowTitle, rowId, description, id]) => ({
        title: rowTitle || '',
        rowId: rowId || '',
...
// Llenamos dynamicButtons con la estructura deseada
list?.forEach(lister => {
    dynamicButtons.push({
        name: 'single_select',
        buttonParamsJson: JSON.stringify({
