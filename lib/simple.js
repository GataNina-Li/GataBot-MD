...
let channelR = canalesInfo[indiceAleatorio]
return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text, contextInfo: { mentionedJid: null, forwardedNewsletterMessageInfo: { newsletterJid: channelRD.id, serverMessageId: '', newsletterName: channelRD.name }, forwardingScore: 1, isForwarded: true, ...options }}, { quoted, ...options })
}
},
