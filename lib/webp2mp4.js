...
async function webp2VideoToJpg(source) {
    let form = new FormData();
    let isUrl = typeof source === 'string' && /^https?:\/\//.test(source);
    const blob = !isUrl && new Blob([source.toArrayBuffer()]);
    form.append('new-image-url', isUrl ? blob : '');
    form.append('new-image', isUrl ? '' : blob, 'video.mp4');
    let res = await fetch('https://ezgif.com/video-to-jpg', {
        method: 'POST',
        body: form
    });
    ...
    let res2 = await fetch('https://ezgif.com/video-to-jpg/' + obj.file, {
        method: 'POST',
        body: form2
    });
