...
const databaseFY = fs.existsSync(fantasyDBPath) ? JSON.parse(fs.readFileSync(fantasyDBPath, 'utf8')) : [];
...
const calculatePrice = (calificacionesPersonaje, character) => {
    ...
    const aumento_por_like = (likes >= 50) ? likes * 0.01 : likes * 0.02;
    const aumento_por_superlike = (superlikes >= 50) ? superlikes * 0.03 : superlikes * 0.05;
    const decremento_por_dislike = dislikes * 0.01;
    ...
    return Math.max(Math.round(nuevoPrecio), 50);
};

const lowCostCharacters = data.infoImg
    .filter(character => character.price !== undefined && character.price >= 0 && character.price <= 700)
    .map(character => {
        const calificacionesPersonaje = databaseFY.flatMap(usuarioObj => (usuarioObj[Object.keys(usuarioObj)[0]].flow || []).filter(voto => voto.character_name === character.name));
