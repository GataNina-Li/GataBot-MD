        ...
        buttons: [
          {
            ...
            buttonText: {displayText: '𓃠 𝗔 𝗨 𝗗 𝗜 𝗢'},
            type: 1
          },
          {
            buttonId: `.ytmp4 ${yt_play[0].url}`,
            buttonText: {displayText: '𓃠 𝗩 𝗜 𝗗 𝗘 𝗢'},
            type: 1
          }
        ],
        viewOnce: true,
...
handler.before = async (m, {conn}) => {
  ...
  const audioApis = [
    {
      url: () => ogmp3.download(userVideoData.url, selectedQuality, 'audio'),
      extract: (data) => ({data: data.result.download, isDirect: false})
    },
    {
      url: () => ytmp3(userVideoData.url),
      extract: (data) => ({data, isDirect: true})
    },
    {
      url: () =>
    ...
    },
    {
      ...
      extract: (data) => ({
        data: data.status ? data.data.download.url : null,
        isDirect: false
      })
    },
    {
  ...
  const videoApis = [
    {
      url: () => ogmp3.download(userVideoData.url, selectedQuality, 'video'),
      extract: (data) => ({data: data.result.download, isDirect: false})
    },
    {
      url: () => ytmp4(userVideoData.url),
