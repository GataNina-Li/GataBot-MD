...
if (!who) return conn.reply(m.chat,  `${lenguajeGB['smsAvisoIIG']()} *𝙐𝙎𝘼𝙍 𝙀𝙇 𝘾𝙊𝙈𝘼𝙉𝘿𝙊 𝘿𝙀 𝙀𝙎𝙏𝘼 𝙁𝙊𝙍𝙈𝘼 𝙀𝙅𝙀𝙈𝙋𝙇𝙊:\n*${usedPrefix + command}* @${m.sender.split('@')[0]}`,  m, {mentions: [m.sender]});
  switch (command) {
case 'addowner':
...
await conn.reply(m.chat, `${lenguajeGB['smsAvisoIIG']()}*𝙀𝙇 𝙉𝙐𝙈𝙀𝙍𝙊 𝙄𝙉𝙂𝙍𝙀𝙎𝘼𝘿𝙊 𝙉𝙊 𝙀𝙓𝙄𝙎𝙏𝙀 𝙀𝙉 𝙇𝘼 𝙇𝙄𝙎𝙏𝘼 𝘿𝙀 𝙊𝙒𝙉𝙀𝙍𝙎.*`, m);
}
break;
}};
