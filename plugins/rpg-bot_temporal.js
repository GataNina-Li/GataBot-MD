...
let enlace2 = { contextInfo: { externalAdReply: { showAdAttribution: true, mediaUrl: yt, mediaType: 'VIDEO', description: '', title: wm, body: ' 𝗦𝘂𝗽𝗲𝗿 𝗕𝗼𝘁-𝗠𝗗 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 ', thumbnailUrl: await(await fetch(img)).buffer(), sourceUrl: yt }}}
...
let texto7 = `${eg}𝙎𝙀 𝙃𝘼 𝙐𝙉𝙄𝘿𝙊 𝘽𝙊𝙏 𝘼𝙇 𝙂𝙍𝙐𝙋𝙊!!!\n\n𝘽𝙊𝙏 𝙃𝘼𝙎 𝙅𝙊𝙄𝙉𝙀𝘿 𝙏𝙃𝙀 𝙂𝙍𝙊𝙐𝙋!!!`
...
await delay(3 * 3000)
let res = await conn.groupAcceptInvite(code) 
conn.reply(m.chat, `${eg} *SE HA UNIDO AL GRUPO!!!*`, m).then(async() => { //Si el Owner se une al Grupo no habrá temporizador
