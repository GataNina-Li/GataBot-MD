...
conn.sendButton(m.chat, `_🥵 Pack 🥵_`, author, url, [['🥵 𝙎𝙄𝙂𝙐𝙄𝙀𝙉𝙏𝙀 | 𝙉𝙀𝙓𝙏 🥵', `${usedPrefix + command}`]], m)
break
case "pack2":  
let url2 = await packgirl[Math.floor(Math.random() * packgirl.length)]
conn.sendButton(m.chat, `_🥵 Pack 2 🥵_`, author, url2, [['🥵 𝙎𝙄𝙂𝙐𝙄𝙀𝙉𝙏𝙀 | 𝙉𝙀𝙓𝙏 🥵', `${usedPrefix + command}`]], m) 
break
case "pack3":
let url3 = await packmen[Math.floor(Math.random() * packmen.length)]
conn.sendButton(m.chat, `_🥵 Pack 3 🥵_`, author, url3, [['🥵 𝙎𝙄𝙂𝙐𝙄𝙀𝙉𝙏𝙀 | 𝙉𝙀𝙓𝙏 🥵', `${usedPrefix + command}`]], m) 
break
case "videoxxx": case "vídeoxxx":    
let url4 = await videosxxxc[Math.floor(Math.random() * videosxxxc.length)] 
await conn.sendButton(m.chat, `*𝘿𝙄𝙎𝙁𝙍𝙐𝙏𝘼 𝙀𝙇 𝙑𝙄𝘿𝙀𝙊 🥵*`, author, url4, [['🥵 𝙎𝙄𝙂𝙐𝙄𝙀𝙉𝙏𝙀 | 𝙉𝙀𝙓𝙏 🥵', `${usedPrefix + command}`]], m)  
break  
...
await conn.sendButton(m.chat, `*𝘿𝙄𝙎𝙁𝙍𝙐𝙏𝘼 𝙀𝙇 𝙑𝙄𝘿𝙀𝙊 🥵*`, author, url5, [['🥵 𝙎𝙄𝙂𝙐𝙄𝙀𝙉𝙏𝙀 | 𝙉𝙀𝙓𝙏 🥵', `${usedPrefix + command}`]], m)    
break    
}}
