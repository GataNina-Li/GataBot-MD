...
return conn.reply(
m.chat,
`*No posee personajes.* Primero compre un personaje usando *${usedPrefix}fantasy* o *${usedPrefix}fy* para cambiarlo por *Tiempo Premium*`,
m
)
}
...
body: `🎟️ Tienes Premium por: ${tiempoPremiumFormateado} `,
sourceUrl: accountsgb,
...
await conn.reply(
m.chat,
`*Has cambiado a ${personaje} por Tiempo premium*\n\n🎟️ *Tiempo premium:* \`\`\`${tiempoPremiumFormateado}\`\`\``,
m,
fake
)
...
await conn.reply(
m.chat,
`*Has cambiado a ${personajesMismaClase.length} Personajes por Tiempo premium\n\n🎟️ *Tiempo premium:* \`\`\`${tiempoTotalFormateado}\`\`\``,
m,
fake
)
let userInDB = fantasyDB.find((userEntry) => userEntry[userId])
