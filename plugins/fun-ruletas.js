...
if (new Date - global.db.data.users[m.sender].juegos < 21600000) return m.reply(`『⏰』𝙒𝙚𝙤𝙣 𝙀𝙨𝙥𝙚𝙧𝙖 : ${msToTime(date - new Date())} 𝙥𝙖𝙧𝙖 𝙫𝙤𝙡𝙫𝙚𝙧 𝙖 𝙟𝙪𝙜𝙖𝙧 `)
...
if (ruleta === 'text5') return m.reply(`𝙒𝙐𝙐𝙐 𝙎𝙀𝙉𝙊𝙍 𝙀𝙎𝙏𝘼 𝘿𝙀 𝙎𝙐𝙀𝙍𝙏𝙀, 𝙑𝘼𝙔𝘼𝙍 𝘼 𝙅𝙐𝙂𝘼𝙍 𝘼𝙇 𝘾𝘼𝙎𝙄𝙉𝙊 🎰\n*𝙂𝘼𝙉𝘼𝙍𝙏𝙀 :* ${diamond} 💎`).catch(global.db.data.users[m.sender].diamond += diamond) 
if (ruleta === 'text6') return m.reply(`👑 𝙂𝙪𝙖𝙖𝙖 𝙚𝙨𝙩𝙖 𝙧𝙚 𝙨𝙪𝙚𝙧𝙩𝙚 𝙂𝙖𝙣𝙖𝙧𝙩𝙚 𝙨𝙚𝙧 𝙥𝙧𝙚𝙢𝙞𝙪𝙢 𝙋𝙤𝙧 : 1 𝙃𝙤𝙧𝙖 ⏰`).catch(global.db.data.users[m.sender].premium += prem) 
