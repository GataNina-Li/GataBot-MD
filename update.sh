...
INSTALL_DP="yarn istall --ignore-scripts && npm install && cd"
...
mv "$DB_FILE" "$HOME" && cd && rm -rf "$HOME/$BOT_DIR" && git clone "$BOT_REPO" && cd "$HOME/$BOT_DIR" && rm -rf $HOME/$BOT_DIR/node_modules && $INSTALL_DP
else
echo -e "${BOLD}${GREEN}No se encontró \"$DB_FILE\" en \"$BOT_DIR\" clonando el repositorio \"$BOT_REPO\" en \"$HOME\"...${RESET}"
cd && rm -rf "$HOME/$BOT_DIR" && git clone "$BOT_REPO" && cd "$HOME/$BOT_DIR" && rm -rf $HOME/$BOT_DIR/node_modules && $INSTALL_DP
fi
